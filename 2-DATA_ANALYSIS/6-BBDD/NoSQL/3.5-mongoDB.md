...
MongoDb es un tipo de bases de datos no relacional `noSQL` orientada a documentos. Se organiza por colecciones. Las columnas y registros se organizan en documentos.

Es un sistema no estructurado o semiestructurado. 

Cantidad de tipos de datos que nos permite almacenar un BSON con respecto a JSON.

- No se pueden almacenar tipo date.
- No se puede almacenar longitud en los datos numericos.

Otra diferencia es el peso. Los JSON tienen una longitud fija de datos.

No tienen una estrucutura predefinida un esquema fijo, son más flexibles. Con una base de datos Mongo es muy facil escalar.

Escalabilildad horizontal: cuando se ha de ampliar una base de datos no relacionales, se puede trabajar en diferentes servidores. La escalabilidad vertical opera en un solo servidor, para escalar se amplian recursos de ese srvidor.

Alto rendimiento, es mas rapido para determinadas consultas en grandes volumenes de datos.
...
- **A - Atomicidad:** Una transacción se ejecuta como una sola unidad; o se completan todos sus pasos o ninguno se ejecuta. Si falla un paso, se revierte todo el proceso para evitar datos parciales. La consulta se hace o no se hace. Se evita tener datos parcialmente almacenados.
- **C - Consistencia:** La transacción garantiza que la base de datos pase de un estado válido a otro estado válido, respetando todas las reglas y restricciones definidas (como que un campo de texto no puede contener números). El dato que almecenamos se almacena con el tipo de datos que se quier. Se tiene que especificar un esquema previamente.
...
- Almacenar mensajes, historial de conversaciones y datos de usuarios. Prima la velocidad.
- **Razón para usar MongoDB**: Su capacidad para manejar datos no estructurados y consultas rápidas.
...
Distintos documentos dentro de un mismo documento.
...
En este ejemplo, los datos del cliente y los productos están embebidos dentro del documento del pedido. Esta toda la información aqui, no es necesario establecer relaciones.
...
No tenemos claves primarias ni foráneas, estamos relacionando directamente. 
...
Es el gestor de base de datos que se trabaja en la nube.
...
mongoshell con docker:
docker exec -it mongo_agc mongosh
...
[ { _id: 'featureCompatibilityVersion', version: '8.2' } ]
admin>

no se puede crear una base de datos sin una colección y no se puede crear una colecccion sin un documento.
no existe comnaod como tal para crear colecciones, en el moento que creemos un documento crearemos una coleccion.
...
Que datos quiero comprar de esa busqueda.

primer argumento: filtro
segundo argumento: proyecciones

```mongoDB
db.<nombreColeccion>.find(<query>,<projeccion>)
//8. Mostrar solo nombre y email (sin _id, por defecto se muestra)
...
**Otros operadores de actualización además del $set (modificadores de campo)**
...
// upsert:true - Si no existe una Lola lo añade
db.clientes.replaceOne(
  {"nombre":"Lola"},
